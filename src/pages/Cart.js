  ...
  return (
    <div className="container" style={{ padding: "40px 20px 70px" }}>
      ...
      <div style={styles.grid} className="cart-grid">
        <div>
          {cart.items.map((item) => (
            <div key={item._id} className="card cart-item-row" style={styles.itemRow}>
              <img src={item.image} alt={item.name} style={styles.itemImg} className="cart-item-img" />
              <div style={{ flex: 1, minWidth: 0 }} className="cart-item-info">
                <Link to={`/product/${item.product}`} style={{ fontWeight: 600 }}>{item.name}</Link>
                {item.size && <p style={{ margin: "4px 0", fontSize: "0.85rem" }}>Size: {item.size}</p>}
                <p style={{ margin: 0, fontWeight: 700, color: "var(--amber-deep)" }}>₹{item.price}</p>
              </div>
              <div style={styles.qtyRow} className="cart-item-qty">
                <button onClick={() => updateItem(item._id, item.quantity - 1)} className="btn-icon">−</button>
                <span style={{ minWidth: 20, textAlign: "center", fontWeight: 700 }}>{item.quantity}</span>
                <button onClick={() => updateItem(item._id, item.quantity + 1)} className="btn-icon">+</button>
              </div>
              <button onClick={() => removeItem(item._id)} style={styles.removeBtn} className="cart-item-remove" aria-label="Remove item">✕</button>
            </div>
          ))}
        </div>
